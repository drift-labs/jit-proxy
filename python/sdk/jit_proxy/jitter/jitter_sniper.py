...
class JitterSniper(BaseJitter):
    ...
    async def create_try_fill(
        ...
        async def try_fill():
            ...
            if is_variant(order.market_type, 'Perp'):
                current_perp_pos = self.drift_client.get_user().get_perp_position(order.market_index)
                if current_perp_pos.base_asset_amount < 0 and is_variant(order.direction, 'Short'):
                    if current_perp_pos.base_asset_amount <= params.min_position:
                        print(f"Order would increase existing short (mkt \
                              {str(order.market_type)}-{order.market_index} \
                              ) too much")
                        del self.ongoing_auctions[order_sig]
                        return
                    elif current_perp_pos.base_asset_amount > 0 and is_variant(order.direction, 'Long'):
                        if current_perp_pos.base_asset_amount >= params.max_position:
                            print(f"Order would increase existing long (mkt \
                              {str(order.market_type)}-{order.market_index} \
                              ) too much")
                        del self.ongoing_auctions[order_sig]
                        return
            ...
            (slot, updated_details) = await self.wait_for_slot_or_cross_or_expiry(target_slot, order, details)

            if slot == -1:
                print("Auction expired without crossing")
                del self.ongoing_auctions[order_sig]
                return
            ...
            bid = convert_to_number(details.oracle_price + params.bid, PRICE_PRECISION) \
                if is_variant(params.price_type, 'Oracle') \
                else convert_to_number(params.bid, PRICE_PRECISION)
            
            ask = convert_to_number(details.oracle_price + params.ask, PRICE_PRECISION) \
                if is_variant(params.price_type, 'Oracle') \
    ...
    def get_auction_and_order_details(self, order: Order) -> AuctionAndOrderDetails:
        ...
        oracle_price = self.drift_client.get_oracle_price_data_for_perp_market(order.market_index) \
            if is_variant(order.market_type, 'Perp') \
            else self.drift_client.get_oracle_price_data_for_spot_market(order.market_index)
        
        maker_order_dir = 'sell' if is_variant(order.direction, 'Long') else 'buy'

        auction_start_price = convert_to_number(
            get_auction_price_for_oracle_offset_auction(order, order.slot, oracle_price.price) \
        ...
        auction_end_price = convert_to_number(
            get_auction_price_for_oracle_offset_auction(order, order.slot + order.auction_duration - 1, oracle_price.price) \
            if is_variant(order.order_type, 'Oracle') \
            else order.auction_end_price,
            PRICE_PRECISION
        )

        bid = convert_to_number(oracle_price.price + params.bid, PRICE_PRECISION) \
            if is_variant(params.price_type, 'Oracle') \
            else convert_to_number(params.bid, PRICE_PRECISION)
        
        ask = convert_to_number(oracle_price.price + params.ask, PRICE_PRECISION) \
            if is_variant(params.price_type, 'Oracle') \
            else convert_to_number(params.ask, PRICE_PRECISION)
        
        slots_until_cross = 0
        will_cross = False
        step_size = (auction_end_price - auction_start_price) // (order.auction_duration - 1)

        while slots_until_cross < order.auction_duration:
            if maker_order_dir == 'buy':
                if convert_to_number(
                    get_auction_price(order, order.slot + slots_until_cross, oracle_price.price),
                    PRICE_PRECISION
                ) <= bid:
                    will_cross = True
                    break
